from ..models import Tag, Review, User, ReviewableNode, Specialization, University, Faculty, Course, Professor
...
def get_tags_by_uid(tags_uids):
    if tags_uids is None: return None
    res = list()
...
def get_tags_from_uids(uids):
    tag_list: [Tag] = list()
    for uid in uids:
        try:
            tag_list.append(Tag.nodes.get(uid=uid))
        except Tag.DoesNotExist:
            return create_mutation_payload(False, error=f"Tag of {uid=} does not exist.")
    return tag_list


def resolve_connect_review_to_tags(_, info, uid: str, uid_tags):
    try:
        review = Review.nodes.get(uid=uid)
        tags = get_tags_from_uids(uid_tags)
        for tag in tags:
            review.tags.connect(tag)
        return create_mutation_payload_review(True, review=review)
    except Review.DoesNotExist:
...
def resolve_disconnect_review_from_tags(_, info, uid, uid_tags):
    try:
        review = Review.nodes.get(uid=uid)
        tags = get_tags_from_uids(uid_tags)
        for tag in tags:
